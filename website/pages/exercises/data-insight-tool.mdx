...
2. Download the four CSV files into the `data/` folder:
   <ul>
     <li><a href="../data/exercise-2/market_penetration.csv" download>market_penetration.csv</a></li>
     <li><a href="../data/exercise-2/demographics.csv" download>demographics.csv</a></li>
     <li><a href="../data/exercise-2/accident_frequency.csv" download>accident_frequency.csv</a></li>
     <li><a href="../data/exercise-2/product_lines.csv" download>product_lines.csv</a></li>
   </ul>
3. Open your terminal in the `data-insight/` folder and start OpenAI Codex
...
- Does OpenAI Codex identify that `region_code` is the join key across files?
...
- Does OpenAI Codex handle the aggregation from product-line level to region level correctly?
...
- Does OpenAI Codex explain its methodology, or does it just present numbers?

> **Tip:** Push OpenAI Codex to refine the analysis: *"Weight the opportunity score more heavily toward regions where MIG already has some presence (market share > 0.1%) -- pure greenfield markets may be too risky."*
...
- Check if OpenAI Codex is filtering to a single year. The `market_penetration.csv` file has data for both 2024 and 2025. Ask Codex: *"How many rows does the merged dataset have? I expected 68 regions."*

### "The opportunity score does not make sense"
- Ask OpenAI Codex to explain its methodology: *"Walk me through how you calculated the opportunity score. What weight did you give each factor?"*
...
- Check if OpenAI Codex used a CDN link for Chart.js. If you are offline, ask: *"Inline the Chart.js library directly in the HTML file instead of using a CDN."*
