...
2. Download all five files into the `data/` folder:
   <ul>
     <li><a href="../data/exercise-4/claims_q1_2025.csv" download>claims_q1_2025.csv</a></li>
     <li><a href="../data/exercise-4/claims_q2_2025.csv" download>claims_q2_2025.csv</a></li>
     <li><a href="../data/exercise-4/claims_q3_2025.csv" download>claims_q3_2025.csv</a></li>
     <li><a href="../data/exercise-4/claims_q4_2025.csv" download>claims_q4_2025.csv</a></li>
     <li><a href="../data/exercise-4/report_template.md" download>report_template.md</a></li>
   </ul>
3. Open your terminal in the `automated-reporting/` folder and start OpenAI Codex
...
- Does OpenAI Codex correctly identify all four quarters?
...
OpenAI Codex generates a self-contained HTML file (`quarterly_report_charts.html`) with five Chart.js charts. The file includes:
...
- Did OpenAI Codex reuse the same report structure, or did it invent a new one?
...
### "OpenAI Codex says it cannot read the files"
...
This can happen if OpenAI Codex misinterprets which column to use. Clarify in your prompt:
...
Check that your browser is not blocking the CDN script (e.g., Chart.js). If you are behind a corporate proxy, ask OpenAI Codex to embed the library inline instead of loading from a CDN.
...
Check which Python version you have (`python --version` or `python3 --version`). The script may require `pandas` -- install it with `pip install pandas` if needed. If OpenAI Codex generated the script using only the standard library, ensure the CSV parsing logic handles the date formats correctly.
