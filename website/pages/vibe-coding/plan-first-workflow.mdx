...
description: How to separate planning from execution when building complex features with OpenAI Codex, so you stay in control of every important decision.
---
...
In the [previous section](/vibe-coding) you used `Shift+Tab` to switch to Plan mode -- a quick way to ask Codex to show its approach before acting. That works well for focused tasks like "add a filter to the dashboard."
...
**Why this matters:**
Without explicit instructions to read thoroughly and document its understanding, Codex might only skim the surface. The `research.md` file gives you something concrete to review -- you can verify Codex actually understands your project *before* it starts changing things.
...
The output should be a detailed markdown file explaining the approach, which files will be created or modified, and any trade-offs. You can point Codex to existing parts of the project as reference (e.g., "Follow the same approach as the existing claims-by-region chart").
...
Repeat this 1 to 3 times until the plan matches your expectations. The markdown file acts as shared, editable context between you and Codex -- much easier to review than scrolling through a long chat history.
...
If Codex makes a mistake during execution, you can provide brief feedback ("the chart should be wider", "wrong folder, move it to the reports section"). Because Codex has the full context of the plan, short corrections are usually enough.
