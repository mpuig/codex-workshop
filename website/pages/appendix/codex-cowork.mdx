---
title: "Appendix A: Codex Cowork"
description: What Codex Cowork is, how it compares to OpenAI Codex and Chat, and how to use plugins, MCP connectors, and cross-session memory for insurance workflows.
---
...
Both Chat and Cowork share conversation, file creation skills (DOCX, PPTX, XLSX, PDF), browser automation (Codex in Chrome), artifacts, connectors (Gmail, GitHub, Slack), and custom MCP servers. The full comparison across all four tools is in the [reference table](#codex-chat-vs-cowork-vs-code-reference-table) at the end of this page.
...
Cowork has a dedicated **Plugins** panel where you can browse, install, and create plugins from a visual interface. Each plugin bundles one or more **skills** -- reusable instruction sets that teach Codex how to approach specific tasks. (For a full guide on creating your own skills, see [Building Skills for Codex](/vibe-coding/building-skills). For OpenAI Codex CLI plugins, see [Appendix C: MCP](/appendix/mcp#plugins).)
...
All types appear in a unified "Connectors" interface with on/off toggles inside Codex Desktop:
...
> **Important:** Chat and Cowork share `codex_desktop_config.json`. Code Tab shares that file plus `~/.codex.json`. Code CLI uses `~/.codex.json` (global) or `.mcp.json` (per-project). Adding an MCP to one tool does not make it available in the others.
...
With Desktop Commander, Codex can install and configure MCP servers, manage dependencies, and set up integrations -- all from a single prompt.
